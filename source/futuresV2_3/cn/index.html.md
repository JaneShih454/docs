...
* 释出 v2.3 API。此更改将于2025年7月16日生效, 更多细节请参阅 [概览](#5060421d15)
* 添加 [Ping/Pong](#pingpong) 机制的说明。
* 释出`notificationApiV4` 於 [通知](#7a66c0d036)
...
## Ping/Pong
为了进行连接检查，我们将每 3 分钟发送一个 ping 帧消息，您需要向我们回复 pong 帧消息以供检查。如果 pong 帧消息未能在 10 分钟内正确回复给我们，我们将强制断开连接。
> 请求
...
接收交易通知，请订阅 `notificationApiV4` 主题，它包含与订单数量变动（例如：原始数量、已成交数量、剩余数量）相关的结构化且定义清晰的字段，有助于与最新的 API 字段更新保持更好的一致性。
